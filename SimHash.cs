...
namespace Analiza_velikih_skupova_podataka_1a
{
    ...
    public class SimHash
    {
        public static void Main(string[] args)
        {
            ...
            for (int j = 0; j < Q; j++)
            {
                ...
                Console.WriteLine(Query(lstHash, I: int.Parse(upit[0]), K: int.Parse(upit[1])));
            }
            // upit_Q-1
        }

        /// <summary>
        /// returns number of documents in  <paramref name="lstHash"/> that differ from  <paramref name="I"/>-th document with hamming distance less or equal <paramref name="K"/>
        /// </summary>
        public static int Query(List<byte[]> lstHash, int I, int K)
        {
            int count = 0;
            var hash = lstHash[I];
            ...
            for (int j = 0; j < lstHash.Count; j++)
            {
                if (j != I)
                {
                    var distance = Hamming.Distance(lstHash[j], hash);
                    if (distance <= K)
                        count++;
                }
            }
